 ...
 * @Auther: hj
 */
object SparkBatchAnalysis3sql2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      """
        |SELECT
        |	country,domain,cnt,rk
        |FROM
        |	(
        |	SELECT
        |		country,domain,cnt,
        |		rank() over(PARTITION by country order by cnt desc) rk
        |	FROM
        |		(
        |		SELECT
        |			country,domain,count(*) cnt
        |		FROM
        ...
        | limit 100
        |""".stripMargin
    ).rdd.coalesce(1).saveAsTextFile("output21")
