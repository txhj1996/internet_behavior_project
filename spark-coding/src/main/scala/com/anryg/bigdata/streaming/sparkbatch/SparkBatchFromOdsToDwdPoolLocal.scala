 ...
 *
 */
object SparkBatchFromOdsToDwdPoolLocal {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val df: DataFrame = spark.sql(
      """
        ...
        | where year=2022 and month=07 and day=29 limit 200000;
        |""".stripMargin
    ).map(row => {
      ...
      //      jedis.select(15)
      val jedis: Jedis = RedisClientUtils.getSingleRedisClient;
      ...
      .toDF("client_ip", "country", "province", "city", "operator", "domain",
        "time", "target_ip", "rcode", "query_type", "authority_record", "add_msg", "dns_ip", "year", "month", "day")

    df.write.partitionBy("year", "month", "day").format("orc").mode("append").saveAsTable("dwd.test1")
