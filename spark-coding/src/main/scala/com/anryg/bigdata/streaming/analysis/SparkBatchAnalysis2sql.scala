...
object SparkBatchAnalysis2sql {
  ...
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      """
        |select
        |	client_ip,domain,cnt,cnt_rank
        |from
        |	(
        |	select
        |		client_ip,domain,cnt,
        |		row_number() over(partition by client_ip order by cnt desc) cnt_rank
        |	from
        |		(
        |		select
        |			client_ip,domain,count(*) cnt
        |		from
        |			(
        |			 select
        |				dwd.client_ip,
        ...
        |	t4.cnt_rank <=10
        |""".stripMargin
    ).rdd.coalesce(1).saveAsTextFile("output8")
