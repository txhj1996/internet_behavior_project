 ...
 *
 */
object SparkBatchFromOdsToDwdLocal {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val df: DataFrame = spark.sql(
      """
        ...
        | where year=2022 and month=07 and day=29 limit 100;
        |""".stripMargin
    ).map(row => {
    ...
    df.write.partitionBy("year", "month", "day").format("orc").mode("append").saveAsTable("dwd.test")
