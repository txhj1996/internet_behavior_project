...
/**
  ...
  *
  */
object DataSkewTestOpti2 {

    def main(args: Array[String]): Unit = {
        System.setProperty("HADOOP_USER_NAME", "hj")
        val conf = new SparkConf()
          .setAppName("DataSkewTestOpti2")
//          .setMaster("local[*]") //本地运行模式，如果提交集群，注释掉这行
        ...
        val firstSaltRdd: RDD[(String, String)] = rawRdd.map(row => {
        ...
        val groupRdd: RDD[(String, Iterable[String])] = firstSaltRdd.groupByKey(100)
...
//                else map.put(clientIP,1)
        ...
        //减盐之后加稍细粒度的盐
        val secondSaltRdd: RDD[(String, util.HashMap[String, Int])] = newMapRdd.map(kv => {
      ...
      val reduceRdd: RDD[(String, util.HashMap[String, Int])] = secondSaltRdd.reduceByKey((map1, map2) => {
      ...
      //第二次减盐
      val finalRDD = reduceRdd.map(kv => {
        val targetIPWithSalt02 = kv._1
        val clientIPMap = kv._2
        if (targetIPWithSalt02.startsWith("1.180.234.207")){
          val targetIP = targetIPWithSalt02.split("-")(0)
        ...
        finalRDD.saveAsTextFile("outputskew2") //结果数据保存目录
