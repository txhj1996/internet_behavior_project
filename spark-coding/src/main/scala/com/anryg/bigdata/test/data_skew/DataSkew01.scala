...
object DataSkew01 {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val rawRDD: RDD[String] = spark.textFile(args(0))//读取数据源

        val filteredRDD: RDD[String] = rawRDD.filter(line => { /**筛选满足需要的数据，已到达数据倾斜的目的*/
        ...
        val reducedRDD: RDD[(String, Array[String])] = filteredRDD.map(line => {/**根据目的ip进行汇总，将访问同一个目的ip的所有客户端ip进行汇总*/
            val array = line.split(",")
            val target_ip = array(3)
        ...
        val targetRDD: RDD[(String, util.HashMap[String, Int])] = reducedRDD.map(kv => {/**将访问同一个目的ip的客户端，再次根据客户端ip进行进一步统计*/
